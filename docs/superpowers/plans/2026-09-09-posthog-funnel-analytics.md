...
- Produces:
  ...
  - `startPostHog(): void` and `capturePageview(): void` — used by the provider.
  - `identifyUser(userId: string): void`
  - `resetPostHog(): void`
  - `<PostHogProvider>{children}</PostHogProvider>`

**One limit of `capture`'s typing, for the tasks that call it.** The generic
prevents pairing one event's name with another's payload only where the two
payload shapes differ. `cart_viewed` and `checkout_started` are structurally
identical — both `{ seller_id, item_count, subtotal_in_cents }` — so `tsc` will
*not* catch a call site that passes the right props under the wrong one of those
two names. Tasks 3 and 5 write exactly those call sites; check the event string
by eye there, because nothing else will.
